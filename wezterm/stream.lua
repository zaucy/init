...
local function reset_terminal_size_and_pos()
	local win = wezterm.gui.gui_windows()[1]

	if win ~= nil then
		win:set_position(0, 0)
		win:set_inner_size(2000, 1370)
	end
end

wezterm.on('gui-attached', reset_terminal_size_and_pos)
wezterm.on('window-config-reloaded', reset_terminal_size_and_pos)
